...
export const Login = async (email, senha) => {
  ...
  try {
    ...
    console.error(erro);
    return { errors: { login: `Erro ao fazer login: ${erro.message}` } };
  }
};
...
export const Signin = async (nome, email, senha) => {
  ...
  try {
    ...
    const dbRefUsuario = db.ref(`usuarios/${user.uid}`);
    const objUsuario = {
      uid: user.uid,
      nome: nome,
      email: user.email,
    ...
    await dbRefUsuario.set(objUsuario);
    await user.sendEmailVerification();

    return { success: { signin: "Conta criada com sucesso. Email de verificação enviado." } };
  } catch (erro) {
    console.error(erro);
    return { errors: { signin: `Erro ao criar conta: ${erro.message}` } };
  }
};
...
/**
 * Verificar Persistência do Login
 */
export const isLoggedIn = async () => {
  const storedUserState = await AsyncStorage.getItem("userLoggedIn");
  return JSON.parse(storedUserState || "false");
};
